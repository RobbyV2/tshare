...
#[derive(Clone)]
struct AppState {
    tunnel_api_base: String,
    tunnel_ws_base: String,
}
...
struct Args {
    ...
    /// Tunnel server API base URL
    #[arg(long, default_value = "http://127.0.0.1:8081")]
    tunnel_api_url: String,

    /// Tunnel server WebSocket base URL
...
async fn main() -> Result<()> {
    ...
    let test_url = format!("{}/api/session/connection-test", args.tunnel_api_url);
    ...
    let app_state = AppState {
        tunnel_api_base: args.tunnel_api_url,
        tunnel_ws_base: args.tunnel_ws_url,
    };
...
async fn session_page(
    ...
    let client = reqwest::Client::new();
    let url = format!("{}/api/session/{}", state.tunnel_api_base, session_id);
...
async fn authenticate_session(
    ...
    // If no passwords are set, authenticate automatically
...
async fn handle_session_ws(
    ...
    ws.on_upgrade(move |socket| proxy_to_tunnel_server(socket, session_id, state))
}

async fn proxy_to_tunnel_server(client_socket: WebSocket, session_id: String, state: AppState) {
    let tunnel_url = format!("{}/ws/web/{}", state.tunnel_ws_base, session_id);
